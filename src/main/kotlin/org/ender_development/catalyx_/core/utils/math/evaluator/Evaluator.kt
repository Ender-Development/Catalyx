...
import org.ender_development.catalyx_.core.utils.math.evaluator.TokenType.*
...
internal class Evaluator() : ExprVisitor<BigDecimal> {
	...
	override fun visitCallExpr(expr: CallExpr): BigDecimal {
		val name = expr.name
		val function = functions[name.lowercase(Locale.ROOT)] ?: throw ExpressionException("Undefined function '$name'")
	...
	override fun visitVariableExpr(expr: VariableExpr): BigDecimal {
		...
		return variables[name.lowercase(Locale.ROOT)] ?: throw ExpressionException("Undefined variable '$name'")
	}
