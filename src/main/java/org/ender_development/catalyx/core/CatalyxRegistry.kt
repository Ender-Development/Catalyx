...
object CatalyxItemRegistry : IRegistry<IItemProvider, Item> {
	override val registry = ObjectOpenHashSet<IItemProvider>()

	@SubscribeEvent
	override fun register(event: RegistryEvent.Register<Item>) {
		Catalyx.LOGGER.debug("Item Registry: ${registry.size} item(s)")
...
object CatalyxBlockRegistry : IRegistry<IBlockProvider, Block> {
	override val registry = ObjectOpenHashSet<IBlockProvider>()

	@SubscribeEvent
	override fun register(event: RegistryEvent.Register<Block>) {
		Catalyx.LOGGER.debug("Block Registry: ${registry.size} block(s)")
		registry.filter { it.isEnabled }.forEach { it.register(event) }
	}

	@SubscribeEvent
	fun registerItemBlocks(event: RegistryEvent.Register<Item>) {
		Catalyx.LOGGER.debug("Item Block Registry: ${registry.size} block item(s)")
		registry.filter { it.isEnabled }.forEach {
